...
func _ready() -> void:
	...
	# 	EnemySpawner.spawn_pawn_near_player(EnemyRegistry.get_key(name), Vector2.UP * 100)
	# 	)

	# _register_command("z", func(amount):
	# 	EnemySpawner.spawn_horde(PawnEnums.NAME.BASIC_ZOMBIE, int(amount))
	# 	)
	...
	# 	for enemy: RuntimeEnemy in enemies:
...
func _register_command(name: String, description: String, usage: String, callable: Callable):
	if _get_command(name) != null:
		push_error("Command with that name already exists.")
		return
	var command: ConsoleCommand = ConsoleCommand.new(name, description, usage, callable)
...
func _handle_console_input(text: String):
	...
	# print("Console: Entered command " + command_name + " with arguments " + str(arguments))

	var command: ConsoleCommand = _get_command(command_name)
	if command != null:
		# commands[command_name].callv(arguments)
		command.callable.callv(arguments)
	else:
		log_message("Command " + command_name + " not found.")
